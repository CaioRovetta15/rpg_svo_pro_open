...
void Reprojector::reprojectFrames(const FramePtr& cur_frame,
    ...
    for (const FramePtr& ref_frame : visible_kfs) {
        ...
        for (size_t i = 0; i < ref_frame->num_features_; ++i) {
            ...
            // may have initialized some 3d points from the ground-truth.
    ...
    if (doesFrameHaveEnoughFeatures(cur_frame)) {
        ...
        // the occupancy such that the depth filter doesn't extract too many new
        // features.
...
bool matchCandidate(const FramePtr& frame, Reprojector::Candidate& c,
                    Matcher& matcher, FeatureWrapper& feature,
                    const double seed_sigma2_thresh) {
    ...
    // other way round is only done if this frame is selected as keyframe.
    feature.type = c.type;
...
bool getCandidate(const FramePtr& cur_frame, const FramePtr& ref_frame,
                  const size_t& ref_index, Reprojector::Candidate& candidate) {
